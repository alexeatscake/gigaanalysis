...
Attributes:
    ...
    popt (np.array): The optimum values for the parameters
    pcov (np.array): The estimated covariance
    results (ga.Data): The optimal values obtained from the fit, will be
        none if full=False when performing the fit
    residuals (ga.Data): The residuals of the fit, will be none
...
The point of this class is to store the results from a gigaanalysis fit so
the arguments are the same as the attributes.
"""
        self.func = func
        self.popt = popt
        self.pcov = pcov
...
predicated y_vales
Args:
    x_vals (np.array): An array of x_vales
Returns:
    y_vals (np.array): An array of precivted y_values
"""
...
def curve_fit(data_set, func, p0=None, full=True, **kwargs):
    """
This is an implementation of spicy.optimize.curve_fit for acting on ga.Data
objects. This performs a least squares fit to the data of a function.
Args:
    data_set (ga.Data): The data to perform the fit on
    func (function): The model function to fit. It must take the x_values as
        the first argument and the parameters to fit as separate remaining
        arguments
    p0 (np.array): Initial guess for the parameters. Is passed to
        scipy.optimize.curve_fit included so it can be addressed positionally
    full (bool default:True): If True fit_result will include results
        and residuals if False they will not be calculated
    kwargs: Keyword arguments are passed to spicy.optimize.curve_fit
Returns:
    fit_result (ga.fit.Fit_result): A gigaanalysis Fit_result object with
